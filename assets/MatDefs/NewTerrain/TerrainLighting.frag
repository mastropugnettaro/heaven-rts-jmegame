...
vec2 computeLighting(in vec3 wvPos, in vec3 wvNorm, in vec3 wvViewDir, in vec3 wvLightDir){
  ...
  vec4 calculateDiffuseBlend(in vec2 texCoord) {
    ...
    //vec4 diffuseColor = texture2D(m_DiffuseMap, texCoord * m_DiffuseMap_0_scale) * 0.7 + texture2D(m_DiffuseMap, texCoord*-0.25 * m_DiffuseMap_0_scale) * 0.3;
  ...
  vec3 calculateNormal(in vec2 texCoord) {
    ...
    vec4 getTriPlanarBlend(in vec4 coords, in vec3 blending, in sampler2D map, in float scale) {
      ...
      //vec4 col1 = texture2D( map, coords.yz * scale) * texture2D( map, coords.yz * scale * -0.25) *4;
      //vec4 col2 = texture2D( map, coords.xz * scale)* texture2D( map, coords.xz * scale * -0.25) *4;
      //vec4 col3 = texture2D( map, coords.xy * scale)* texture2D( map, coords.xy * scale * -0.25) *4;
